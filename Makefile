...
test: test-gateway test-core ## Run all Go tests

test-gateway: ## Run gateway tests
	cd backend/gateway && GOCACHE=$(GOCACHE) go test ./...

test-core: ## Run core-service tests
	cd backend/core-service && GOCACHE=$(GOCACHE) go test ./...
...
bundle: ## Pack a backup set into one portable file, usage: make bundle [RUN_ID=2026-09-23_12-00-00]
